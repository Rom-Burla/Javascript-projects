...
appHeading.textContent = "Cities & Color Picker";
root.appendChild(appHeading);
let appInstruction = document.createElement("p");
appInstruction.textContent =
  "Write RGB values from 0 - 255 if the value exceeds 255 it replaces itself with 255 if the value is bellow 0 it replaces itself with 0, Pick 3 colors and add them to favorites: First for visited cities, Second for will visit cities, Third for will not visit cities";
...
let favoriteCheck = () => {
  ...
  if (favouriteColorArr.length === 3) {
    favouriteColorArr.forEach((color) => {
      localStorage.setItem(color.nickname, JSON.stringify(color));
    });
    window.location.href = "../cities.html";
  } else {
    favouriteColorArr.length === 1
      ? (appInstruction.textContent =
          "please pick 2 more colors and add them to favorites")
      : favouriteColorArr.length === 2
      ? (appInstruction.textContent =
          "please pick 1 more color and add it to favorites")
      : false;
  }
};
favouriteFirstColorBtn.addEventListener("click", favoriteCheck);
favouriteSecondColorBtn.addEventListener("click", favoriteCheck);
